#this is the same as the four.py except we are clearing the screen after each event and standardizing the framerate
...
while not done: #starting the event loop: which waits for user input and reacts to events(their actions)
    ...
    if pressed[pygame.K_SPACE]: print("space")
    if pressed[pygame.K_UP]: y-=3
    if pressed[pygame.K_DOWN]: y+=3
    if pressed[pygame.K_LEFT]: x-=3
    if pressed[pygame.K_RIGHT]: x+=3

    screen.fill((0, 0, 0)) #reseting the screen to black
